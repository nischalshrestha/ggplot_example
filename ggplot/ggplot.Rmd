...
tutorial_event_recorder <- function(tutorial_id, tutorial_version, user_id, event, data) {
  if (!is.null(data[['code']])) {
    ...
    write.table(x, file = "executions.csv", sep = ",", col.names = !file.exists("executions.csv"), row.names = FALSE, append=TRUE)
  }
}
...
In this experiment, you will recreate three different plots using R's `ggplot` library. Before that, let's do a quick introduction on how to plot using `ggplot`.
